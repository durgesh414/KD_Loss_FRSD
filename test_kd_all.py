...
def parse_args():
    ...
    parser.add_argument('--folder', 
                        help='the parent folder containing project directories to be tested')
...
def main_worker(config):
    ...
    for proj_dir in config['project']['proj_dirs']:
        ...
        # check if proj_dir is a directory
        if not osp.isdir(proj_dir):
            print(f"Skipping {proj_dir} because it is not a directory.")
            continue

        # load config
        config_path = osp.join(proj_dir, 'student_config.yml')

        if not osp.exists(config_path):
            print(f"Skipping {proj_dir} because it does not contain a 'student_config.yml'.")
            continue

        with open(config_path, 'r') as f:
            test_config = yaml.load(f, yaml.SafeLoader)

        test_config['project']['proj_dir'] = proj_dir
        test_config['project']['model_dir'] = osp.join(proj_dir, "models/")
        ...
        bkb_paths = [
            osp.join(model_dir, 'student_backbone_{}.pth'.format(save_iter))
...
if __name__ == '__main__':
    ...
    if args.folder:
        ...
        project_dirs = [osp.join(args.folder, d) for d in os.listdir(args.folder) if osp.isdir(osp.join(args.folder, d))]
        config['project']['proj_dirs'] = project_dirs
