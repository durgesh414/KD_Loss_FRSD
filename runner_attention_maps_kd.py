...
class IterRunner():
    ...
    def __init__(self, student_config, teacher_config, train_loader, val_loaders, student_model, teacher_model):
        ...
        # student_model = self.student_model['backbone']['net']
        # print("Student Layers using named_modules():")
        # for name, layer in student_model.named_modules():
        #     print(f"Layer name: {name}")
        #     # print(layer)
        #     print("-" * 30)

        # teacher_model = self.teacher_model['backbone']['net']
        # print("Teacher Layers using named_modules():")
    ...
    def train(self):
        ...
        # Register hooks for the student's layer4.1 conv1 and conv2
        student_block = self.student_model['backbone']['net'].layer4[1]  # Accessing the second block (index 1) in layer4
        student_block.conv1.register_forward_hook(get_activation('student_conv1', student_activations))
        student_block.conv2.register_forward_hook(get_activation('student_conv2', student_activations))

        # Register hooks for the teacher's layer4.2 conv1 and conv2
        teacher_block = self.teacher_model['backbone']['net'].layer4[2]  # Accessing the third block (index 2) in layer4
        teacher_block.conv1.register_forward_hook(get_activation('teacher_conv1', teacher_activations))
        teacher_block.conv2.register_forward_hook(get_activation('teacher_conv2', teacher_activations))
        ...
        # Compute attention maps and match between student and teacher
        attention_transfer_loss = 0
        for name in ['student_conv1', 'student_conv2']:
            student_attention_map = compute_attention_map(student_activations[name])
            teacher_attention_map = compute_attention_map(teacher_activations[name.replace('student', 'teacher')])
            # print(f"Computing attention maps for: {name} and corresponding teacher layer")

            # Resize teacher attention map to match student attention map dimensions, if needed
            if student_attention_map.shape != teacher_attention_map.shape:
                teacher_attention_map = F.interpolate(teacher_attention_map, size=student_attention_map.shape[2:], mode='bilinear', align_corners=False)
